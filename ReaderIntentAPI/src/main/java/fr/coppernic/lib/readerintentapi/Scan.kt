...
const val ACTION_RFID_SUCCESS = "org.scan.read.success"
const val ACTION_RFID_ERROR = "org.scan.read.failed"
...
fun getScanIntent(readerType: Type, scan: Boolean): Intent {
    ...
    return when (readerType) {
        Type.BARCODE -> {
            ...
            scanIntent.action = if (scan) INTENT_ACTION_SCAN else INTENT_ACTION_SCAN_STOP
            scanIntent
        }
        Type.RFID_ICLASS -> {
